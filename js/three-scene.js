...
class Zenith3DStage {
  ...
  buildTeaScene() {
    ...
    teapotGroup.position.set(-1.8, 2.45, 0);
    teapotGroup.rotation.z = -0.32; // Natural pour angle pointing toward cup
    teapotGroup.rotation.y = 0.08;
    ...
    // Curved Pouring Spout: curves up from body, then points DOWN towards cup center
    const p0 = new THREE.Vector3(0.7, 0.05, 0);
    const p1 = new THREE.Vector3(1.15, 0.38, 0);
    const p2 = new THREE.Vector3(1.5, 0.22, 0);
    const p3 = new THREE.Vector3(1.72, -0.06, 0); // Mouth of spout pointing downward!
    const spoutCurve = new THREE.CatmullRomCurve3([p0, p1, p2, p3]);
    const spoutGeo = new THREE.TubeGeometry(spoutCurve, 32, 0.12, 16, false);
    const spout = new THREE.Mesh(spoutGeo, ceramicCeladonMat);
    spout.castShadow = true;
    teapotGroup.add(spout);

    // Exact Spout Tip Anchor for liquid emission
    const spoutTipAnchor = new THREE.Object3D();
    spoutTipAnchor.position.copy(p3);
    teapotGroup.add(spoutTipAnchor);
    this.teaSpoutTipAnchor = spoutTipAnchor;

    // Refined spout lip ring
    const lipGeo = new THREE.TorusGeometry(0.12, 0.02, 12, 24);
    const lip = new THREE.Mesh(lipGeo, ceramicCeladonMat);
    lip.position.copy(p3);
    lip.rotation.z = -0.55;
    teapotGroup.add(lip);
    ...
    this.teaStreamMat = streamMat;
    this.teaStreamMesh = new THREE.Mesh(new THREE.BufferGeometry(), streamMat);
    this.teaStreamMesh.visible = false;
    this.teaGroup.add(this.teaStreamMesh);
    this.updateTeaStreamGeometry();
  ...
  buildCoffeeScene() {
    ...
    // Tip anchor at the very mouth of the gooseneck spout
    const coffeeTipAnchor = new THREE.Object3D();
    coffeeTipAnchor.position.set(2.1, 0.6, 0.2);
    kettleGroup.add(coffeeTipAnchor);
    this.coffeeSpoutTipAnchor = coffeeTipAnchor;
  ...
  /**
   * Helper: Retrieve precise 3D world coordinates of Kyusu Spout Tip
   */
  getTeaSpoutWorldPos() {
    const v = new THREE.Vector3();
    if (this.teaSpoutTipAnchor) {
      this.teaGroup.updateMatrixWorld(true);
      this.teaSpoutTipAnchor.getWorldPosition(v);
    } else {
      v.set(-0.24, 1.90, 0);
    }
    return v;
  ...
  /**
   * Helper: Dynamically rebuild tea stream geometry connecting teapot spout directly to cup center
   */
  updateTeaStreamGeometry() {
    if (!this.teaGroup) return;
    const startPos = this.getTeaSpoutWorldPos();
    const targetPos = new THREE.Vector3(0, 1.05, 0);
    const midPoint = new THREE.Vector3(
      startPos.x * 0.42,
      (startPos.y + targetPos.y) * 0.52,
      startPos.z * 0.5
    );
    const streamCurve = new THREE.QuadraticBezierCurve3(startPos, midPoint, targetPos);
    if (this.teaStreamMesh) {
      if (this.teaStreamMesh.geometry) this.teaStreamMesh.geometry.dispose();
      this.teaStreamMesh.geometry = new THREE.TubeGeometry(streamCurve, 28, 0.046, 12, false);
    }
  }

  /**
   * Action 1: Trigger Single Tea Drop (Accurately emitted from Kyusu Spout mouth)
   */
  triggerTeaDrop() {
    if (this.isPouringTea) return;
    this.isPouringTea = true;

    // Gentle physical tilt of the Kyusu teapot forward
    if (window.gsap && this.teapotGroup) {
      gsap.to(this.teapotGroup.rotation, {
        z: -0.44,
        duration: 0.22,
        yoyo: true,
        repeat: 1,
    ...
    // Reset drop position EXACTLY at the opening mouth of the teapot spout
    const spoutPos = this.getTeaSpoutWorldPos();
    this.teaDropMesh.position.copy(spoutPos);
    this.teaDropMesh.scale.set(0.8, 1.4, 0.8);
    this.teaDropMesh.visible = true;

    // Animate falling drop along gravity trajectory into teacup center (0, 1.05, 0)
    if (window.gsap) {
    ...
    if (enable) {
      if (window.gsap && this.teapotGroup) {
        gsap.to(this.teapotGroup.rotation, {
          ...
          duration: 0.4,
          ease: "power2.out",
          onUpdate: () => this.updateTeaStreamGeometry()
        });
      }
      this.updateTeaStreamGeometry();
      if (this.teaStreamMesh) this.teaStreamMesh.visible = true;
      if (window.zenithAudio) window.zenithAudio.startPourSound();
      if (window.zenithAudio) window.zenithAudio.startSteamHiss(0.12);
    } else {
      if (this.teaStreamMesh) this.teaStreamMesh.visible = false;
      if (window.gsap && this.teapotGroup) {
        gsap.to(this.teapotGroup.rotation, {
          z: -0.32,
          duration: 0.4,
  ...
  /**
   * Helper: Retrieve precise 3D world coordinates of Gooseneck Spout Tip
   */
  getCoffeeSpoutWorldPos() {
    const v = new THREE.Vector3();
    if (this.coffeeSpoutTipAnchor) {
      this.coffeeGroup.updateMatrixWorld(true);
      this.coffeeSpoutTipAnchor.getWorldPosition(v);
    } else {
      v.set(-0.35, 3.2, 0);
    }
    return v;
  ...
  /**
   * Helper: Dynamically rebuild coffee stream geometry from kettle spout into V60 dripper
   */
  updateCoffeeStreamGeometry() {
    if (!this.coffeeGroup) return;
    const startPos = this.getCoffeeSpoutWorldPos();
    const targetPos = new THREE.Vector3(0, 2.45, 0);
    const midPoint = new THREE.Vector3(
      startPos.x * 0.45,
      (startPos.y + targetPos.y) * 0.52,
      startPos.z * 0.5
    ...
    if (this.coffeeStreamMesh) {
      if (this.coffeeStreamMesh.geometry) this.coffeeStreamMesh.geometry.dispose();
      this.coffeeStreamMesh.geometry = new THREE.TubeGeometry(curve, 24, 0.04, 10, false);
    }
  }
  ...
  triggerCoffeeBrew() {
    ...
    // 1. Tilt Gooseneck kettle forward gracefully
    if (window.gsap && this.kettleGroup) {
      gsap.to(this.kettleGroup.rotation, {
        z: -0.55,
        duration: 0.8,
        ease: "power2.out",
        onUpdate: () => this.updateCoffeeStreamGeometry()
      });
    }

    // 2. Start pour stream & sound
    setTimeout(() => {
      this.updateCoffeeStreamGeometry();
